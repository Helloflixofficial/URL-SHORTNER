...
interface CampaignRow { id: string; name: string; websiteUrl: string; budget: number; spent: number; status: number; adType: number; createdAt: Date }
...
const AD_TYPE = { 1: 'Interstitial', 2: 'Banner', 3: 'Popup' }

export default function MemberCampaignsTable({ campaigns, total, page, pageSize, searchQuery }: Props) {
  ...
  const updateStatus = async (id: string, action: 'pause' | 'resume') => {
    const res = await fetch('/api/campaigns/mass', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action, ids: [id] })
    })
    if (res.ok) { toast.success(`Campaign ${action === 'pause' ? 'paused' : 'resumed'}`); router.refresh() }
    else toast.error('Failed to update campaign')
  }

  const massAction = async (action: 'pause' | 'resume' | 'delete') => {
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {selectedIds.size > 0 && (
            <>
              <Button variant="outline" size="sm" className="glass text-xs" onClick={() => massAction('resume')}>
        ...
        <Table>
          ...
          <TableBody>
            {campaigns.length === 0 ? (
              ...
              const typeName = AD_TYPE[c.adType as keyof typeof AD_TYPE] ?? 'Unknown'
              return (
                <TableRow key={c.id} className="border-border/30 table-row-hover group">
                  ...
                  <TableCell><span className="text-sm font-medium">{typeName}</span></TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="glass border-border">
                        {c.status === 1 && <DropdownMenuItem className="text-blue-400 cursor-pointer" onClick={() => updateStatus(c.id, 'pause')}><Pause className="w-3.5 h-3.5 mr-2" />Pause</DropdownMenuItem>}
                        {c.status === 2 && <DropdownMenuItem className="text-emerald-400 cursor-pointer" onClick={() => updateStatus(c.id, 'resume')}><Play className="w-3.5 h-3.5 mr-2" />Resume</DropdownMenuItem>}
                        <DropdownMenuItem className="text-red-400 cursor-pointer" onClick={() => { if(confirm('Delete?')) fetch(`/api/campaigns/mass`, { method: 'POST', body: JSON.stringify({ action: 'delete', ids: [c.id] }) }).then(() => router.refresh()) }}><Trash2 className="w-3.5 h-3.5 mr-2" />Delete</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" disabled={page <= 1} asChild className="glass border-border/50">
              <Link href={`/campaigns?page=${page - 1}${q ? `&q=${q}` : ''}${currentStatus ? `&status=${currentStatus}` : ''}`}><ChevronLeft className="w-4 h-4" /></Link>
            </Button>
            <Button variant="outline" size="sm" disabled={page >= totalPages} asChild className="glass border-border/50">
              <Link href={`/campaigns?page=${page + 1}${q ? `&q=${q}` : ''}${currentStatus ? `&status=${currentStatus}` : ''}`}><ChevronRight className="w-4 h-4" /></Link>
            </Button>
          </div>
        </div>
      )}
