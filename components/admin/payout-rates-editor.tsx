...
const AD_TYPES = [
  { id: 'interstitial', label: 'Interstitial CPM', key: 'payout_rates_interstitial' },
...
export default function PayoutRatesEditor({ interstitial, banner, popup }: Props) {
  ...
  const [matrices, setMatrices] = useState<Record<string, RateMatrix>>({
    interstitial, banner, popup
  })

  const handleUpdate = (adType: string, country: string, device: string, value: string) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end mb-2">
        <Button onClick={save} disabled={saving} className="btn-glow gradient-bg-primary text-primary-foreground">
          {saving ? 'Saving...' : <><Save className="w-4 h-4 mr-2" />Save Rates</>}
        </Button>
      </div>
      ...
      <Tabs defaultValue="interstitial">
        <TabsList className="glass border border-border/50 p-1 h-auto mb-4">
          {AD_TYPES.map(t => (
            <TabsTrigger key={t.id} value={t.id} className="text-sm data-[state=active]:bg-primary data-[state=active]:text-primary-foreground rounded-lg">
              {t.label}
            </TabsTrigger>
          ))}
        </TabsList>
        ...
        {AD_TYPES.map(type => {
          ...
          const countries = Object.keys(matrix).sort((a, b) => a === 'all' ? -1 : b === 'all' ? 1 : a.localeCompare(b))
            ...
            <TabsContent key={type.id} value={type.id}>
              <Card className="glass border-border/50">
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Input 
                      ...
                      maxLength={2}
                    />
                    <Button size="sm" variant="outline" className="h-8 glass" onClick={() => addCountry(type.id)}>
                ...
                <CardContent>
                      ...
                      <TableBody>
                        {countries.map(c => (
                          <TableRow key={c} className="border-border/30">
                            <TableCell className="font-semibold">{c === 'all' ? 'Worldwide Deal (all)' : c.toUpperCase()}</TableCell>
                            {DEVICE_TIERS.map(d => (
                              <TableCell key={d}>
                                <div className="relative">
                                  ...
                                  <Input 
                                    ...
                                    step="0.0001"
                                    value={matrix[c]?.[d] || 0}
                                    onChange={(e) => handleUpdate(type.id, c, d, e.target.value)}
                            ...
                            <TableCell>
                              {c !== 'all' && (
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => removeCountry(type.id, c)}>
                  ...
                  <p className="text-xs text-muted-foreground mt-3">Values represent CPM (Cost Per 1000 views). "all" is used as the fallback rate for countries not explicitly listed.</p>
                </CardContent>
              </Card>
            </TabsContent>
          )
