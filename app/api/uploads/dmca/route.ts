...
// GET /api/uploads/dmca — list DMCA reports for this account
export async function GET() {
  ...
  try {
    const url = new URL(`${DATANODES_BASE}/api/files/dmca`)
    url.searchParams.set('key', DATANODES_KEY)
    const res = await fetch(url.toString(), { cache: 'no-store' })
    if (!res.ok) throw new Error(`DataNodes responded with ${res.status}`)
    return NextResponse.json(await res.json())
  } catch (err) {
    console.error('[uploads/dmca GET] DataNodes error:', err)
    return NextResponse.json({ error: 'Failed to fetch DMCA reports' }, { status: 502 })
  }
}
