...
export default async function DashboardPage() {
  ...
  const [
    ...
    prisma.user.findUnique({
      where: { id: userId },
      select: { balance: true, totalEarned: true, username: true },
    }),
    ...
    prisma.statistic.aggregate({
      where: { userId, reason: 1, createdAt: { gte: startOfToday } },
      _sum: { publisherPrice: true },
    }),
  ...
  const deviceStats = rawDeviceStats.map((d) => ({
    device: d.device,
    count: d._count._all,
  }));
  ...
  const dailyStats = Object.entries(dayMap).map(([date, count]) => ({
    date,
    count,
  }));
  ...
  const stats = [
    {
      label: "Total Clicks",
      value: (totalHits._sum.hits ?? 0).toLocaleString(),
      icon: Eye,
      chipClass: "icon-chip-cyan",
    },
    {
      label: "Earnings Today",
      value: `$${(todayEarnings._sum.publisherPrice ?? 0).toFixed(4)}`,
      icon: DollarSign,
      chipClass: "icon-chip-amber",
    },
    {
      label: "Total Earned",
      value: `$${(earnings._sum.publisherPrice ?? 0).toFixed(4)}`,
      icon: TrendingUp,
  ...
  const adTypeLabel = (t: number) =>
    ({ 0: "Direct", 1: "Interstitial", 2: "Banner", 3: "Random" })[t] ??
    "Unknown";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Button
          asChild
          className="btn-glow gradient-bg-primary text-primary-foreground w-full sm:w-auto"
        >
          <Link href="/links/new">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
        {stats.map((s) => (
          <Card
            key={s.label}
            className="glass border-border/50 hover:border-primary/20 transition-all stat-card"
          >
            <CardContent className="pt-6">
              <div className="flex items-center justify-between mb-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.chipClass}`}
                >
        ...
        <CardContent>
          {recentLinks.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <Button
                asChild
                className="mt-4 btn-glow gradient-bg-primary text-primary-foreground"
              >
                <Link href="/links/new">
                  <Plus className="w-4 h-4 mr-2" />
                  Create Link
            ...
            <div className="divide-y divide-border/30">
              {recentLinks.map((link) => (
                <div
                  key={link.id}
                  className="py-3 flex items-center gap-4 table-row-hover rounded-lg px-2"
                >
