...
export default async function AdminReportsPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-black" style={{ fontFamily: 'Space Grotesk, sans-serif' }}><span className="gradient-text">Reports</span></h1>
        <p className="text-muted-foreground mt-1">Platform analytics — last 30 days</p></div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total Views', value: totalViews.toLocaleString(), icon: Eye, color: '#7c3aed' },
          { label: 'Paid Views', value: paidViews.toLocaleString(), icon: BarChart3, color: '#06b6d4' },
          { label: 'Payouts', value: `$${(totalEarnings._sum.publisherPrice ?? 0).toFixed(2)}`, icon: TrendingUp, color: '#10b981' },
        ].map(s => (
              ...
              {topCountries.map((c, i) => (
                <div key={c.country} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${(c._count / (topCountries[0]?._count ?? 1)) * 100}%`, background: 'var(--gradient-primary)' }} />
                  </div>
